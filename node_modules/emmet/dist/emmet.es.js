 ...
 */
function resolveSnippets(node, parentAncestors, parentConfig) {
    const stack = [];
    const resolve = (child, ancestors, config) => {
        ...
        const abbr = parseAbbreviation(snippet, config);
        stack.push(snippet);
 ...
 */
function getBlockName(ancestors, depth = 0) {
    const maxParentIx = 0;
...
function template(text) {
    ...
    while (scanner.pos < scanner.text.length) {
        pos = scanner.pos;
        if (placeholder = consumePlaceholder$1(scanner)) {
 ...
 */
function consumePlaceholder$1(scanner) {
    if (peek$1(scanner) === 91 /* Start */) {
        const start = ++scanner.pos;
        let namePos = start;
        let afterPos = start;
        let stack = 1;
...
const formatters = { html, haml, slim: pug, pug: pug$1 };
/**
...
function parse(abbr, config) {
    if (typeof abbr === 'string') {
        ...
        if (config.options['jsx.enabled']) {
            parseOpt = Object.assign({}, parseOpt, { jsx: true });
        }
    ...
    walk(abbr, resolveSnippets, config);
...
function stringValue(scanner) {
    ...
    const start = scanner.pos;
    let finished = false;
    if (ch === 39 /* SingleQuote */ || ch === 34 /* DoubleQuote */) {
        scanner.pos++;
        while (!scanner.eof()) {
...
function consumeNumber(stream) {
    ...
    stream.eatWhile(isNumber$1);
    const prevPos = stream.pos;
    if (stream.eat(46 /* Dot */) && !stream.eatWhile(isNumber$1)) {
...
function parseValue(value) {
    return parse$1(value.trim(), opt)[0].value;
}
...
function resolveNode(node, snippets, config) {
    if (!resolveGradient(node, config)) {
        ...
        if (config.context) {
            ...
            const snippet = snippets.find(s => s.type === "Property" /* Property */ && s.property === config.context);
...
function resolveGradient(node, config) {
    ...
    if (gradientFn || node.name === 'lg') {
        if (!gradientFn) {
            ...
            gradientFn = Object.assign({}, gradientFn, { name: 'linear-gradient' });
        }
        if (!config.context) {
        ...
        for (const token of cssVal.value) {
            if (token.type === 'Literal') {
                ...
                if (match && match.type === 'FunctionCall') {
                    value.push(Object.assign({}, match, { arguments: token.arguments.concat(match.arguments.slice(token.arguments.length)) }));
                }
                else {
                    value.push(token);
                }
...
function setNodeAsText(node, text) {
    node.name = void 0;
    node.value = [cssValue(literal$3(text))];
    return node;
}
/**
...
function mergedData(type, syntax, key, config, globals = {}) {
    ...
    return Object.assign({}, defaultConfig[key], (typeDefaults && typeDefaults[key]), (syntaxDefaults && syntaxDefaults[key]), (typeOverride && typeOverride[key]), (syntaxOverride && syntaxOverride[key]), config[key]);
}
...
function isUnquotedValue(ch) {
    return !isNaN(ch) && ch !== 61 /* Equals */ && !isWhiteSpace$4(ch) && !isQuote$2(ch);
}
...
function extractAbbreviation(line, pos = line.length, options = {}) {
    ...
    const opt = Object.assign({}, defaultOptions$1, options);
...
function markup(abbr, config) {
    return stringify$1(parse(abbr, config), config);
}
/**
...
export { extractAbbreviation as extract, markup, parse as parseMarkup, parse$2 as parseStylesheet, convertSnippets as parseStylesheetSnippets, resolveConfig, stringify$1 as stringifyMarkup, css as stringifyStylesheet, stylesheet };
